...
function GetMeteo(id,span)
{
    span.setAttribute("style","font-size: 12px;")
    var apikey = "7ef59beefeb70c5638b6aca294c1ef85";
    var proxyurl = "https://cors-anywhere.herokuapp.com/";
    link = proxyurl+"https://api.openweathermap.org/data/2.5/weather?units=metric&id="+id+"&appid="+apikey;
    loadDocMeteo(link,span);
}

function loadDocMeteo(link,span)
{
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            DisplayMeteoResult(this.responseText,span);
        }
    };
...
function DisplayMeteoResult(obj,span)
{
    meteoObj = JSON.parse(obj);
    
    
    span.innerHTML = " ("+meteoObj['main']['temp']+"°C)";
}
...
function DisplayDestinationIndex(destination)
{
    ...
    var spanMeteo = document.createElement("span");
    GetMeteo(destination['id_meteo'],spanMeteo);
    
    h3.appendChild(spanMeteo);
